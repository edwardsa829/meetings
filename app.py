...
class JWMeetings(rumps.App):
    ...
    def playlists(self, _):
        ...
        try:
            response = requests.get(f"https://wol.jw.org/en/wol/dt/r1/lp-e/{schedule}", headers=headers, timeout=5)
        except Exception as e:
            print(str(e))
...
class Meetings(object):
    ...
    def weekends(self):
        ...
        article = requests.get(link, headers=headers, timeout=5)
    ...
    def midweek(self):
        ...
        try:
            treasures_content = str(requests.get(treasures_link, headers=headers, timeout=5).text)
        except Exception as e:
            print(str(e))
        ...
        try:
            for x in range(len(ministry)):
                ...
                try:
                    ministry_content = str(requests.get(ministry_link, headers=headers, timeout=5).text)
        ...
        for link in living_links:
            try:
                living_content = str(requests.get(link, headers=headers, timeout=5).text)
        ...
        try:
            response1 = str(requests.get(bs_links[0], headers=headers, timeout=5).text)
        except Exception as e:
            print(str(e))
        ...
        if len(bs_links) > 1:
            ...
            try:
                response2 = str(requests.get(bs_links[1], headers=headers, timeout=5).text)
            except Exception as e:
                print(str(e))
                res = rumps.alert("Error", "There was a problem. Make sure you are connected to the internet or try again later.\nWould you like to send a report to the developer?", ok="Send Report..", cancel="Cancel")
                if res == 1:
